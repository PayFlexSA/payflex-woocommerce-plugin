...
final class OptionsTest extends PF_TestCase
{
    ...
    public function test_non_array_option_is_coerced_to_an_empty_array(): void
    {
        ...
        $this->assertSame([], payflex_get_option());
        $this->assertFalse(payflex_get_option('enabled'));
    }

    public function test_falsy_saved_values_are_returned_verbatim(): void
    {
        $this->set_settings(['widget_theme' => '']);
        $this->assertSame('', payflex_get_option('widget_theme'));
    }

    /**
     * payflex_get_option(false) is the documented "give me everything" call, so
    ...
    {
        $settings = $this->set_settings();
        $this->assertSame($settings, payflex_get_option(false));
    }
}
