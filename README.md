# Payflex WooCommerce Plugin — Developer Guide

Internal documentation for the Payflex payment gateway plugin. **This file is
excluded from the WordPress.org deploy** (see `.distignore`) — the public listing
is `readme.txt`.
...
Requires PHP 8.3+ and Composer. No database, no WordPress install, no network
access — the suite runs against stubs and takes well under a second.

```bash
cd wp-content/plugins/payflex-payment-gateway

composer install          # once, or after composer.json changes
...
# One test
vendor/bin/phpunit --filter test_a_declined_payment_fails_the_order

# List every test as it runs
vendor/bin/phpunit --testdox

# Syntax-check every shipped PHP file (what CI's lint job does)
composer lint
```
...
2. **Decide what is actually wrong:**
   ...
   - **A version mismatch** (`version-check` failed) — the three version
     locations disagree, or they disagree with the tag. See
     [Cutting a release](#cutting-a-release).
...
4. **Re-point the release at the fixed commit.** On GitHub: Releases → the
   release → *Edit* → set the target to the new commit → *Update release*.
   Publishing does not re-fire on edit, so:

   - Actions → *Deploy payflex to WordPress.org* → *Run workflow* is not
     available (the workflow is release-triggered only), so instead **delete
     the release and its tag, then publish a fresh release** on the fixed
     commit. That re-fires the workflow cleanly.
...
The version appears in **three** places and the suite fails if they disagree:

| Location | What reads it |
| --- | --- |
| `partpay.php` → `Version:` header | WordPress, and WordPress.org's published version |
| `includes/class-wc-gateway-payflex.php` → `private $version` | Sent to Payflex as `merchantSystemInformation.plugin_version`; shown on the support page |
| `readme.txt` → `Stable tag:` | WordPress.org listing |

Checklist:

1. Update all three to the same value.
2. Add a `= x.y.z =` changelog entry to `readme.txt`.
3. Bump `WC tested up to:` in `partpay.php` if it was verified against a newer
   WooCommerce.
4. `composer test` locally.
5. Merge to `main`, confirm `Tests` is green.
6. Tag and publish a GitHub release as `vx.y.z` (or `x.y.z` — both are accepted).
7. Watch the deploy run. `deploy` skipped means a gate failed; see above.
